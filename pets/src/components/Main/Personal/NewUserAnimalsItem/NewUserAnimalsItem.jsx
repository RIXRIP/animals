...
const NewUserAnimalsItem = (props) => {
    ...
    return (
        <>
            <NavLink className={styles.navLink} to={path}><button className={styles.animalButtons} onClick={recordAnimal}> <div><img className={styles.animalPhoto} src={props.animalPhoto} alt="not found"/></div>{props.id}. {props.name}</button></NavLink>
        </>
    )
}
