...
export const postAnimalTC = (inputName, inputSpecies, inputBreed, inputPhoto, userID, inputDescription) => {
    return (dispatch) => {
        animalsAPI.postNewAnimal(inputName, inputSpecies, inputBreed, inputPhoto, userID, inputDescription);
    }
}
export const updateTotalAnimalsTC = (totalAnimals) => {
    return (dispatch) => {
...
export const paginationTC =(currentPage,pageSize)=>{
    return (dispatch)=>{
        animalsAPI.getAnimals(currentPage, pageSize).then(data => {
            dispatch(setAnimals(data));
        });
        animalsAPI.getTotalCount().then(data => {
            dispatch(setTotalAnimals(data.animalsCount));

        });
    }
}
export const onPageChangedTC = (pageNumber,pageSize) => {
    return(dispatch)=>{
        dispatch(setCurrentPage(pageNumber));
        animalsAPI.getAnimals(pageNumber, pageSize).then(data => {
            dispatch(setAnimals(data));
        });
    }
}
